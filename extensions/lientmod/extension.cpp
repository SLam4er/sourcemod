...
clientmod g_clientmod;		/**< Global singleton for extension's main interface */
...
SMEXT_LINK(&g_clientmod);

extern sp_nativeinfo_t g_CSNatives[];

ISDKTools *g_pSDKTools = NULL;

bool clientmod::SDK_OnLoad(char *error, size_t maxlength, bool late)
{
#if SOURCE_ENGINE != SE_CSGO
	if (strcmp(g_pSM->GetGameFolderName(), "clientmod") != 0)
	{
		ke::SafeStrcpy(error, maxlength, "Cannot Load ClientMod Extension on mods other than ClientMod and CS:GO");
		return false;
	}
#endif
	...
	char conf_error[255];
	if (!gameconfs->LoadGameConfigFile("sm-clientmod.games", &g_pGameConf, conf_error, sizeof(conf_error)))
	{
		if (error)
		{
			ke::SafeSprintf(error, maxlength, "Could not read sm-clientmod.games: %s", conf_error);
		}
		return false;
	}

	sharesys->AddNatives(myself, g_CSNatives);
	sharesys->RegisterLibrary(myself, "clientmod");
...
bool clientmod::SDK_OnMetamodLoad(ISmmAPI *ismm, char *error, size_t maxlen, bool late)
{
...
void clientmod::SDK_OnAllLoaded()
{
...
bool clientmod::QueryRunning(char *error, size_t maxlength)
{
...
bool clientmod::QueryInterfaceDrop(SMInterface *pInterface)
{
...
void clientmod::NotifyInterfaceDrop(SMInterface *pInterface)
{
	g_RegNatives.UnregisterAll();
}
...
bool clientmod::ProcessCommandTarget(cmd_target_info_t *info)
{
	int max_clients;
	IPlayerInfo *pInfo;
...
const char *clientmod::GetExtensionVerString()
{
	return SOURCEMOD_VERSION;
}

const char *clientmod::GetExtensionDateString()
{
	return SOURCEMOD_BUILD_TIME;
}

void clientmod::OnPluginLoaded(IPlugin *plugin)
{
...
void clientmod::OnPluginUnloaded(IPlugin *plugin)
{
	if (m_WeaponPriceDetourEnabled && !g_pPriceForward->GetFunctionCount())
	{
		RemoveWeaponPriceDetour();
